"""Multilingual golden-utterance end-to-end coverage for ovos-skill-diagnostics.

Intent match is asserted directly off the ``ovos.intent.matched`` bus
event's ``data.intent_name`` field (already stripped of the ``.intent``
suffix), matching this repo's own (now-replaced) ``test_golden_utterances.py``.
Capture ends at ``mycroft.skill.handler.start``, right after intent binding
fires, so a row never depends on a handler body finishing.
